...
_ntrs = [
    ...
    (14, '渐进的接触', 'B', ['N27','N28','N29','N30','N31','N32','N33','N34','N35','N36','N37','N38','N39','N40','N41','N42','N43','N44'],
     '玲口交→玲裸足→桌下之手→菲裸足→桌下之口→艾玛手交→树后秘密→双人共享→艾扑克→酒后口交→凯尔乳交→酒后之夜→凯尔再战→黎恩安排→凯尔山洞→装睡代价→全身被舔→不准丢下'),
    (15, '跨线', 'C', ['N45','N46','N47','N48','N49','N50','N51','N52','N53','N54','N55','N56','N57','N58','N59','N60','N61'],
     '★乔治的唇→被干软提问→白丝勾引雷恩→第二次低语者→温柔疗愈选凯尔→第一次被插入(凯尔)→翌日疗愈→走廊绯红→犹豫说不要→桌下吞精→黎恩提议再试→早晨喂奶不痛了→雷恩第二次→弯腰弧度→开裆裤袜→试衣镜前→隐奸设计'),
    (16, '放开', 'D前半', ['N62','N63','N64','N65','N66','N67','N68','N69','N70','N71','N72'],
     '圈子开放→劳拉乳交→姐妹足交→艾德里安本番→重新填满→亚莉莎→四人共享→劳拉白袜→艾玛传送门→草药咖啡拘束→菲的早晨'),
    (17, '享受', 'D后半', ['N73','N74','N75','N76','N77','N78','N79','N80','N81','N82','N83'],
     '醉诱→温泉羞辱→密林即兴→菲本番→镜湖亡妻→镜湖内射→胜利3P→温泉晕厥→艾玛吊带袜→亚莉莎本番→花田3P'),
    (18, '极限', 'E前半', ['N84','N85','N86','N87','N88','N89','N90','N91','N92','N93','N94'],
     '亚尔缇娜足交→欲望之镜→玲游戏本番→主动手交→艾德里安舌→雷恩裸足→劳拉骑士→雷恩跪礼→第一次自己决定→凯尔独白→凯尔观摩'),
    (19, '反转', 'E后半', ['N95','N96','N97','N98','N99','N100','N101','N102','N103','N104'],
     '主动口交→桌下之手→桌下之口→凯尔邀请→乔治震动棒→腐化迷雾→黑暗乳交→催情茶→低语者轮奸→洗干净回归'),
    (20, '终局', '终局', ['N105','N106','N107','N108'],
     '低语者轮奸→洗干净回归→她的情书→终局抉择'),
    (21, '后日谈', '后日谈', [],
     '（NTRS路线专属后日谈——暂无事件）'),
]
...
# —— 第23-31章：被动NTR路线 ——
_pn = [
    (23, 'PN：裂痕', 'A', ['PN1','PN2','PN3','PN4','PN5'],
     '黎恩缺席→Thalion诱惑→乔治误解→亚莉莎对比→寂寞释放'),
    (24, 'PN：腐蚀', 'A→B', ['PN6','PN7','PN8','PN9','PN10','PN11'],
     'Thalion强迫摸乳→指交→甜言蜜语→腐蚀低语→药与酒→肉体展示'),
    (25, 'PN：沦陷', 'B', ['PN12','PN13','PN14','PN15','PN16','PN17'],
     '被迫手交→指交陷阱→被迫口交→乳间耻辱→半推半就→骑虎难下'),
    (26, 'PN：堕落', 'B→C', ['PN18','PN19','PN20','PN21'],
     '★堕落之夜(Thalion插入)→足交玷污→耳边低语→不敢出声'),
    (27, 'PN：沉溺', 'C→D', ['PN22','PN23','PN24','PN25'],
     '再次找上门→窗外影子→趁他睡着→半开的门'),
    (28, 'PN：觉醒', 'D', ['PN26','PN27','PN28','PN29'],
     '主动口交→镜中自己→野外暴露→主动索求'),
    (29, 'PN：放肆', 'D+', ['PN30','PN31','PN32','PN33'],
     '主动邀约→故意遗留→Thalion伪装→桌下之手'),
    (30, 'PN：巅峰', 'D+巅峰', ['PN34','PN35','PN36'],
     '桌面下的脚→桌下之口→隐乳交'),
    (31, 'PN：终局', '终局', ['PN37'],
     '与Thalion的最终战斗'),
]
