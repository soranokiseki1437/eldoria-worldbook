...
**Logged**: 2026-08-10T10:00:00Z
**Priority**: high
**Status**: pending
**Area**: docs

### Summary
"新增章节各字段字数不超过优秀章节（166《玲的挑战——口交对决》）"的约束**仅适用于内容字段**（核心/章节任务/情境/终止条件），**名称字段无字数限制**。执行阶段3方案时我擅自把4个超10字的名称收窄（245.5/253.5/255.5/268.5），用户纠正"章节名称没有字数限制"后全部恢复方案原名。

### Details
用户指令原文："新增章节内各字段内容字数绝不能超过该优秀章节"。我理解为含"名称"字段（166名称=10字），于是把用户已审阅批准的名称缩短，并在报告里声明了4处改名。用户纠正：名称不受限。修正：恢复4个方案原名（北线的测绘——猎兵与鬼之力/水车下的暗影——腐化史莱姆/东线的暗火——预警石板报警/树根下的火——腐化反扑），文件名与内容同步恢复。

### Suggested Action
执行章节方案时：**已批准的名称/结构字段一律不动**；"字数不超过优秀章节"只核对情境每条、核心、章节任务、终止条件等正文内容字段。对字段约束有歧义时先问用户，不擅自改已批准内容。

### Metadata
- Source: user_feedback
- Related Files: docs/story/3：渐进接触/245.5/253.5/255.5/268.5（名称恢复）
- Tags: 字数限制, 名称字段, 章节方案执行
...
### Summary
**章节终止条件3 = 摄影机撤出，不写任何新剧情，也不能包含章节已有内容**（不得复述情境或条件2里已发生的事件）。规则明文在 `docs/story/_条件修复方案.md` §1.4.1，166《玲的挑战——口交对决》的条件3"玲离开后两人也回了木屋，营地在夜色中安静下来"即为收束画面范例。我初稿把"地图交到菲娜手里""净化复检完成""次日标记防御图"这类事件复述写成条件3，被用户纠正。

### Details
正确结构（对照166与规则文档）：
- 条件1=前置骨架（谁在哪、什么状态，不重复情境细节）
- 条件2=核心事件（叙事节拍序列，句号分隔，写"发生了什么"）
- 条件3=收束画面：环境音/物件特写/时间推进/情感落点，如"火堆的柴烧尽了，营地只剩巡逻的脚步声。地图躺在菲娜的木桌上，墨迹干了。"
修正流程：把原条件3的事件内容融合进条件2，条件3重写为撤出画面。同时顺带清理条件字段全局禁令：禁破折号、禁否定形式（不/没/非/无，如"无声撤离"→"安静撤离"）、禁比喻、禁章节编号引用。

### Suggested Action
写终止条件前先读 `_条件修复方案.md` §1.2-1.4，条件3按"摄影机撤出"模式写，禁止事件复述。

### Metadata
- Source: user_feedback
- Related Files: docs/story/3：渐进接触/245.5/253.5/255.5/258.5/268.5（终止条件重写）, docs/story/_条件修复方案.md
- Tags: 终止条件, 条件3收束, 摄影机撤出, 否定词禁令
...
### Summary
用户说"自我优化"时指**调用 self-improvement 技能**（记录教训到 .learnings/），不是让我优化章节内容本身。术语澄清："self-improve/自我优化"= 走 self-improvement 技能流程。

### Details
用户在"终止条件重写"完成后输入"自我优化"，我理解为对5章内容做全规则自检优化并开始重读全文；用户打断纠正"我的意思是self-improve"，即调用 self-improvement 技能把本次会话的纠正记录为学习条目。会话中产生教训：名称无字数限制（LRN-20260810-001）、条件3收束规则（LRN-20260810-002）。

### Suggested Action
用户使用"自我优化/self-improve/self-improvement"等词时，直接调用 self-improvement 技能；对章节内容的优化会用"重写/精修/优化章节"等明确措辞。

### Metadata
- Source: user_feedback
- Related Files: .learnings/LEARNINGS.md
- Tags: 术语, self-improvement, 流程
