...
> **SillyTavern 世界书** · 幽暗奇幻 × 史诗羁绊 × 单线纯爱共享融合线
>
...
| **路线** | 单线纯爱共享融合线 — 共享时刻 ⇄ 回归时刻，844章线性叙事 |
...
│
├── docs/                           # 权威数据源
│   ├── story/                      # ★ 章节TXT文件（844个，唯一权威）
│   │   ├── 0：序章/       (70)    苏醒→告白→第三者登场
│   │   ├── 1：试探和暧昧/ (44)    坦白→注视→第一次共享
│   │   ├── 2：挑逗和接受/ (35)    主动展示→初次口交
│   │   ├── 3：渐进接触/   (128)   足交→手交→乳交→口交→隐奸
│   │   ├── 4：跨线/       (130)   首次插入→自主选择
│   │   ├── 5：享受和掌控/ (183)   主动设计→多P→轮奸→极限
│   │   ├── 6：放纵/       (133)   主动服务→极限→低语者
│   │   ├── 7：终局/       (20)    决战前夜→净化→情书
│   │   └── 8：后日谈/     (101)   自由探索阶段
│   ├── chapter/                     # 始终触发条目+概念文件（8个）
│   ├── character/                   # 角色档案（14角色+总览）
│   ├── creature/                    # 生物设定（19种）
│   ├── location/                    # 地点设定（29处）
│   ├── npc/                         # NPC档案（15人+总览）
│   ├── world/                       # 世界观概念
...
```
docs/story/{章节}/*.TXT   ← ★ 章节唯一权威源（844个独立文件）
    │
