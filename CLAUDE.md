...
### V10.2 经验总结（2026-06-28）

**本次变更**：188→211事件（+23），新增NPC×2（狼人月语者NPC10+蜥蜴人鳞母NPC11），阶段8后日谈从0到6事件，新增肛交线（2事件）+低语者净化线（3事件）+狼人DP线。

**关键教训**：

| # | 教训 | 说明 |
|---|------|------|
| 1 | **方案先行** | 大规模新增事件（>3个）必须写方案文档→审查→Rule6扫描→用户确认→再创建TXT。本次4个方案文档（阶段1-6/肛交狼人/低语者净化）证明了此流程有效 |
| 2 | **破折号限制** | 已写入_TEMPLATE_RULES.md §4.5。新事件情境段破折号（——）不超过20个。替代：句号断句>逗号轻顿>直接续接 |
| 3 | **Rule6扫描自动化** | 方案和TXT文件均需正则扫描`不是.{0,50}(而是|是[^故员为认...])`模式。角色对话中的"不是"属于豁免 |
| 4 | **renumber防误伤** | renumber脚本会将情境文本中的数字（如"150cm""30分钟"）误判为事件编号引用。renumber后必须检查度量单位+年龄段描述是否被篡改 |
| 5 | **Edit工具空格问题** | Edit工具对中英文混排文件的缩进（tab/space）敏感。含中文的替换建议用Python脚本而非Edit工具 |
| 6 | **方案与TXT分离** | 方案文件（`*新事件方案.md`）保留在根目录作为设计记录。TXT文件创建时须同步精简破折号+修复Rule6 |

**当前状态**：211事件·9章节·12角色·27地点·22生物·11NPC·构建通过·Rule6情境字段已清零
