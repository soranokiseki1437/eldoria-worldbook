...
| `章节任务` | ✅ | 1句话核心任务，2026-06-29全量补写（302章） |
| `章节终止条件` | ✅ | 条件列表（\n分隔编号），2026-06-29全量补写（302章） |
...
| # | 变更 | 说明 |
|---|------|------|
| 12 | **302章全量补写** | `章节任务`+`章节终止条件`从仅ch224有→全部302章补齐 |
| 13 | **_TEMPLATE/RULES同步** | 模板字段表+范例补齐新字段，去重复§3.4/§3.5，事件数169→302 |

**当前状态**：434条目·302章节·9阶段·12角色·构建通过·11字段精简Object格式·JSON 1.3MB·章节任务/终止条件全覆盖
