...
**本次变更**：188→211事件（+23），新增NPC×2，后日谈章节从0到6事件，新增多条叙事线覆盖新领域。

**关键教训**：

| # | 教训 | 说明 |
|---|------|------|
| 1 | **方案先行** | 大规模新增事件（>3个）必须写方案文档→审查→Rule6扫描→用户确认→再创建TXT。本次实践证明了此流程有效 |
...
| 6 | **方案与TXT分离** | 方案文件保留在根目录作为设计记录。TXT文件创建时须同步精简破折号+修复Rule6 |
